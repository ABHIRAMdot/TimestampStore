...
def validate_status_transition(current_status, new_staus):
    ...
    if new_staus not in allowed_transitions:
        return (
            False,
            f"Cannot change status from '{current_status}' to '{new_staus}'. Please follow the proper order flow.",
        )
    return True, None
...
def update_order_status(order, new_status, changed_by=None, notes=None):
    ...
    order.items.exclude(
        status__in=["cancelled", "returned", "return_requested"]
    ).update(
        status=new_status,
...
def get_order_statistics():
    ...
    stats = Order.objects.aggregate(
        ...
        shipped_orders=Count(
            "id", filter=Q(status__in=["shipped", "out_for-delivery"])
        ),
...
def get_low_stock_products(threshold=10):
    ...
    return (
        Product_varients.objects.filter(
            stock__lte=threshold, is__listed=True, product__is_listed=True
        )
...
def get_out_of_stock_products():
    ...
    return Product_varients.objects.filter(
        stock=0, is_listed=True, product__is_listed=True
    ).select_related("product")
