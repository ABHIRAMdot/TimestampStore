...
def cancel_order_view(request, order_id):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            success, message = cancel_order(
                order, reason=reason, cancelled_by=request.user
            )
...
def cancel_order_item_view(request, item_id):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            reason = form.cleaned_data.get("reason", "").strip() or "No reason provided"
            success, message = item.cancel_item(
                reason=reason, cancelled_by=request.user
            )
...
def return_order_item_view(request, item_id):
    ...
    if not item.can_be_returned:
        messages.error(
            request,
            "This item cannot be returned. Either it's not delivered yet or the 7-day return window has expired.",
        )
        return redirect("user_order_detail", order_id=item.order.order_id)
    ...
    if request.method == "POST":
        ...
        if "apply_coupon" in request.POST:
            ...
            if buy_now_item:
                # for buy now
                cart_total = (
                    Decimal(str(buy_now_item["price"])) * buy_now_item["quantity"]
                )
            else:
                # for cart
...
def place_order(request):
    ...
    if buy_now_item:
        ...
        if not wallet_only and total_amount > Decimal("1000"):
            messages.error(
                request,
                "Cash on Delivery is not available for orders  above ₹1,000. Please pay online or use wallet.",
            )
            return redirect("checkout")
    ...
    for item in cart_items:
        ...
        if item.variant.stock <= 0 or item.variant.stock < item.quantity:
            messages.error(
                request, f"Not enough stock for {item.product.product_name}."
            )
            return redirect("checkout")
    ...
    if not wallet_only and total_amount > Decimal("1000"):
        messages.error(
            request,
            "Cash on Delivery is not available for orders above ₹1,000. Please select pay online or use wallet",
        )
        return redirect("checkout")
