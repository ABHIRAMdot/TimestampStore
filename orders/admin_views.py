...
def admin_order_detail(request, order_id):
    ...
    order = get_object_or_404(
        Order.objects.select_related(
            "user", "shipping_address", "cancelled_by"
        ).prefetch_related(
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            success, message = update_order_status(
                order, new_status, changed_by=request.user, notes=notes
            )
...
def admin_inventory_management(request):
    ...
    ).filter(is_listed=True, product__is_listed=True)
...
def admin_return_requests_list(request):
    ...
    return_requests = (
        OrderItem.objects.filter(status="return_requested")
        .select_related("order__user", "product", "variant")
