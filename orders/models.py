...
class Order(models.Model):
    ...
    def get_cancellable_items(self):
        ...
        return self.items.filter(status__in=["pending", "confirmed", "processing"])
    ...
    def calculate_totals(self):
        ...
        self.discount_amount = sum(
            item.discount_amount * item.quantity for item in active_items
        )
...
class OrderStatusHistory(models.Model):
    ...
    changed_by = models.ForeignKey(
        Account, on_delete=models.SET_NULL, null=True, related_name="status_changes"
    )
