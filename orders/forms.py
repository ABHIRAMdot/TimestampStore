    ...
    def clean(self):
        ...
        if len(reason) < 10:
            raise forms.ValidationError(
                "Please provide a more detailed reason (at least 10 character)."
            )
...
class AdminOrderStatusForm(forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if self.instance and self.instance.pk:
            ...
            self.fields["status"].choices = allowed_statuses.get(
                current_status, Order.STATUS_CHOICES
            )

            if current_status in ["cancelled", "returned"]:
                self.fields["status"].help_text = (
                    f"This order is {current_status}. Status cannot be changed further."
                )
