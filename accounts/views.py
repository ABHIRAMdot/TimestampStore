...
def register(request):
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            expiry_minutes = (
                _otp_expiry_minutes()
            )  # calling the OTP expiring fucntion here
            message = (
                f"Hello {first_name},\n\n"
...
def verify_otp(request):
            ...
            if entered_otp == stored_otp:
                ...
                try:
                    ...
                    if referred_by_code:
                        try:
                            # find the referrer
                            referrer = Account.objects.filter(
                            ...
                            if referrer and referrer != user:
                                ...
                                success, msg, reward = create_referral_reward(
    ...
    return render(
        request,
        "email/otp_verify.html",
        {"form": form, "email": email, "time_remaining": time_remaining},
    )
...
def reset_password(request):
    ...
    if user.otp_created_at:
        ...
        ):  # total_seconds() timedeltas default method to convert the time difference to total second as a float.
            minutes = int(remaining.total_seconds() // 60)
            seconds = int(remaining.total_seconds() % 60)
            time_remaining = f"{minutes}:{seconds:02d}"
    ...
    if request.method == "POST":
        ...
        if not otp or not new_password or not confirm_password:
            ...
            return render(
                ...
                {"email": email, "time_remaining": time_remaining},
            )
...
def resend_otp(request):
    ...
    request.session.modified = (
        True  # save  the modified session after changing the above
    )
...
def profile(request):
    ...
    breadcrumbs = [{"label": "Home", "url": reverse("home")}, {"label": "Profile"}]
...
def change_email(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            )  # changing it to standerdised string format
...
def verify_email_change_otp(request):
    ...
    context = {"form": form, "new_email": new_email, "time_remaining": time_remaining}
