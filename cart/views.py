            ...
            return JsonResponse(
                {
                    ...
                    "cart_count": cart_item.cart.get_item_count(),
                    "line_discount": str(
                        offer_data["original_price"] - offer_data["final_price"]
                    ),
                    "item_savings": str(
                        (offer_data["original_price"] - offer_data["final_price"])
                        * cart_item.quantity
...
def cart_view(request):
    ...
    for item in cart_items:
        if (
            not item.variant.product.is_listed
            or not item.variant.is_listed
    ...
    subtotal_before_discount = sum(
        item.original_price * item.quantity for item in cart_items
    )

    total_savings = sum(
        (item.original_price - item.final_price) * item.quantity for item in cart_items
    )
...
def add_to_cart(request):
    ...
    if quantity > CartItem.MAX_QUANTITY_PER_PRODUCT:
        messages.error(
            request,
            f"Maximum {CartItem.MAX_QUANTITY_PER_PRODUCT} items allowed per product.",
        )
        return redirect("product_detail", slug=product.slug)
    ...
    with transaction.atomic():
        ...
        if not created:
            ...
            messages.success(
                request, f"Increased quantity of {product.product_name} in your cart."
            )
        else:
    ...
    try:
        ...
        if (
            not product
            or not product.is_listed
            or (product.category and not product.category.is_listed)
        ):
            ...
            return render(
                request, "errors/product_unavailable.html", context, status=410
            )
