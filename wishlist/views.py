...
def wishlist_view(request):
    ...
    wishlist_items = (
        wishlist.items.select_related("product", "variant", "product__category")
...
def add_to_wishlist(request):
    ...
    with transaction.atomic():
        ...
        if created:
            ...
            messages.info(
                request, f"{product.product_name} is already in your wishlist."
            )
...
def toggle_wishlist(request):
    ...
    try:
        ...
        if not variant:
            return JsonResponse(
                {"success": False, "message": "No available variants for this product."}
            )
...
def move_to_cart(request):
    ...
    with transaction.atomic():
        ...
        # Get current price
        current_price = get_discounted_price(
            wishlist_item.product, wishlist_item.variant
        )
        ...
        cart_item, created = CartItem.objects.get_or_create(
            ...
            defaults={"quantity": 1, "price": current_price},
        )
        ...
        if not created:
            ...
            messages.success(
                request, f"{wishlist_item.product.product_name} moved to cart."
            )
...
def move_all_to_cart(request):
    ...
    if skipped_items:
        messages.warning(
            request, f"Some items couldn't be moved: {', '.join(skipped_items)}"
        )
