...
def product_list(request):
    ...
    products = (
        ...
        .prefetch_related("varients__images")
        .order_by("-created_at")
    )
    # search filter
    if search_query:
    ...
    main_categories = Category.objects.filter(is_listed=True, parent=None).order_by(
        "category_name"
    )

    # get all sub categories for filter dropdown
    subcategories = Category.objects.filter(
        is_listed=True, parent__isnull=False
...
def add_product(request):
    ...
    categories = (
        Category.objects.filter(is_listed=True, parent__isnull=False)
        .select_related("parent")
        .order_by("parent__category_name", "category_name")
    )
    ...
    if request.method == "POST":
        ...
        for i in range(len(colours)):
            if not colours[i] or not prices[i]:
                messages.error(
                    request, f" Varient {i+1}: colour and price are required."
                )
                return render(
                    request,
                    "add_product.html",
        ...
        if Product.objects.filter(slug=slug).exists():
            messages.error(
                request, f'Slug "{slug}" already exists. Please use a different slug.'
            )
            return render(
                request,
                "add_product.html",
        ...
        try:
            with transaction.atomic():
                ...
                messages.success(
                    request, f'Product "{product_name}" added successfully.'
                )
...
def edit_product(request, product_id):
    ...
    if request.method == "POST":
        ...
        if (
            Product.objects.filter(product_name__iexact=product_name)
        ...
        try:
            with transaction.atomic():
                ...
                if delete_variants:
                    Product_varients.objects.filter(
                        id__in=delete_variants, product=product
                    ).delete()

                # update existing variants
                for i in range(len(variant_ids)):
                    if variant_ids[i] and str(variant_ids[i]) not in delete_variants:
                        variant_obj = Product_varients.objects.get(
                            id=variant_ids[i], product=product
                        )
                        ...
                        if primary_image_key in request.POST:
                            ...
                            if new_primary_id:
                                ...
                                VariantImage.objects.filter(variant=variant_obj).update(
                                    is_primary=False
                                )
                                # set new primary
                                VariantImage.objects.filter(
                                    id=new_primary_id, variant=variant_obj
                ...
                if len(colours) > new_variant_start:
                    for i in range(new_variant_start, len(colours)):
                        if colours[i] and prices[i]:
                            ...
                            images = (
                                request.FILES.getlist(f"variant_images_{i}[]") or []
                            )
                            if len(images) >= 3:
                                for index, img in enumerate(images):
                                    VariantImage.objects.create(
                                ...
                                raise ValueError(
                                    f"Variant {i+1} must have at least 3 images"
                                )
                ...
                for idx, variant in enumerate(all_variants, 1):
                    ...
                    if image_count < 3:
                        raise ValueError(
                            f"Variant {idx} ({variant.colour}) must have at least 3 images (currently has {image_count})"
                        )

                messages.success(
                    request, f'Product "{product_name}" updated successfully.'
                )
                return redirect("product_list")
        except Exception as e:
    ...
    if request.method == "POST":
        ...
        if action == "add":
            ...
            variant = get_object_or_404(
                Product_varients, id=variant_id, product=product
            )
