...
def add_money_create_order(request):
    ...
    if amount < MIN_AMOUNT:
        return JsonResponse(
            {"status": "error", "message": f"Minimum amount is ₹{MIN_AMOUNT}."},
            status=400,
        )

    if amount > MAX_AMOUNT:
        return JsonResponse(
            {"status": "error", "message": f"Maximum amount is ₹{MAX_AMOUNT}."},
            status=400,
        )
...
def add_money_verify_payment(request):
    ...
    if not pending or pending.get("razorpay_order_id") != razorpay_order_id:
        return JsonResponse(
            {"status": "error", "message": "No matching pending recharge found."},
            status=400,
        )
    ...
    try:
        ...
        return JsonResponse(
            {"status": "error", "message": "Failed to verify payment amount"},
            status=400,
        )
    ...
    if not wallet:
        ...
        return JsonResponse(
            {"status": "error", "message": "Failed to credit wallet."}, status=500
        )
