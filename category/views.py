...
def category_list(request):
    ...
    categories = (
        Category.objects.all().select_related("parent").order_by("-created_at")
    )  # get all categories inluding is_listed =False
...
def add_category(request):
    ...
    main_categories = Category.objects.filter(parent=None, is_listed=True).order_by(
        "category_name"
    )
    ...
    if request.method == "POST":
        ...
        if parent_id == "":
            if category_name.lower() not in ["men", "women"]:
                messages.error(
                    request, "You must select a parent category (Men or Women)."
                )
                return render(
                    request,
                    "add_category.html",
        ...
        if Category.objects.filter(slug=slug).exists():
            messages.error(
                request, f'Slug "{slug}" already exists. Please a different slug.'
            )
            return render(
                request,
                "add_category.html",
...
def edit_category(request, category_id):
    ...
    main_categories = Category.objects.filter(parent=None, is_listed=True).exclude(
        id=category_id
    )
...
def toggle_category_status(request, category_id):
    ...
    if category.is_listed:
        ...
        messages.success(
            request, f'Category "{category.category_name}" has been unlisted(deleted)'
        )
    else:
        category.is_listed = True
        messages.success(
            request, f'Category "{category.category_name}" has been listed(restored)'
        )
